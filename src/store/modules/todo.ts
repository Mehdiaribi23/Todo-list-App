// Importation de la fonction uuidv4 pour la génération d'identifiants uniques
import { v4 as uuidv4 } from "uuid";

// Importation des types et interfaces nécessaires depuis d'autres fichiers
...
// Interface représentant l'état global du store
...
const rootActions: ActionTree<RootStateStorable, RootStateStorable> = {
  ...
  addItem(ctx, newItem: ITodo) {
    ...
    try {
      ...
      ctx.commit("setError", { show: true, message: "Impossible de créer l'élément" });
    }
  },
  editItem(ctx, item: ITodo) {
    ...
    try {
      ...
      ctx.commit("setError", {
        show: true,
        message: "Impossible de sauvegarder les données éditées de l'élément",
      });
    }
  },
  deleteItem(ctx, id: string) {
    ...
    try {
      ...
      ctx.commit("setError", { show: true, message: "Impossible de supprimer l'élément" });
    }
  },
